...
class P2PUtil: NSObject {
    ...
    class func createTextFieldWith(Frame frame:CGRect, BoardStyle boardStyle:UITextBorderStyle, PlaceHolder placeHolder:String?, BackgroundColor backgroundColor:UIColor?, TintColor tineColor:UIColor?, IsPWD isPwd:Bool) -> UITextField{
    ...
    //base64加密
    class func encodingBase64(String str:String) -> Int {
        let data = str.data(using: String.Encoding.utf8)
        let base64 = Int((data?.base64EncodedString(options: .lineLength64Characters))!)!
        return base64
    }
    ...
    //判断手机号码合法性
    class func isValidPhoneNum(MobilePhone phone:String) ->Bool {
        if phone.count != 11 {
            return false
        }
        
        //中国移动CM，中国联通CU，中国电信CT
        ...
        if regextestcm.evaluate(with: phone) || regextestcu.evaluate(with: phone) || regextestct.evaluate(with: phone){
            return true
        } else {
            return false
        }
