...
class P2PProgressView: UIView {

    var mainPathWidth:CGFloat = 0.0
    
    lazy var proLabel:UILabel? = {
        let label = P2PUtil.createLabelWith(Text: "", Frame: CGRect(x: 0, y: 0, width: self.bounds.size.width, height: self.bounds.size.height), TextColor: UIColor.white, Font: self.bounds.size.width/6, TextAligtment: .center)
    ...
    func dreawCircleWith(Progress pro:Int){
        progress = pro
        proLabelNum = 0
